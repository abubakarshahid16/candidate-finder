  ...
  const search = async (event: { preventDefault: () => void }) => {
    ...
    try {
      const response = await fetch(
        'http://localhost:3001/api/v1/candidate-search',
        {
          method: 'POST',
      ...
      <main className="ml-60 min-h-screen p-8">
        {screen === 'settings' ? (
          ...
          <>
            ...
            >
              ...
              <Field label="Minimum experience">
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, experienceMin: e.target.value })
                  }
                />
              </Field>
              <Field label="Maximum experience">
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, experienceMax: e.target.value })
                  }
                />
              </Field>
              ...
              <Field label="Optional authorized profile/provider URL">
                <input
                  ...
                  placeholder="https://provider.example/profile"
                />
              </Field>
              <div className="md:col-span-2">
                <Field label="Optional search prompt">
                  <textarea
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    placeholder="Example: Find senior data engineers with strong Python and SQL experience in Saudi Arabia."
                    rows={3}
                    className="block w-full resize-y rounded-lg border border-[#cbdad5] bg-white p-3 font-normal text-[#183d39]"
                  />
                </Field>
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-semibold text-slate-600">
                  ...
                  <div className="mt-2 rounded-lg border border-dashed border-[#9bbdb0] bg-[#f7fbf8] p-4">
                    <input
                      type="file"
                      accept=".txt,.md,.text"
                      onChange={(event) => {
                        ...
                        reader.onload = () => {
                          if (typeof reader.result === 'string') setPrompt(reader.result);
                        };
                    ...
                    <p className="mt-2 text-xs font-normal text-slate-500">
                      Upload a text or Markdown job description. Its contents
                      will be loaded into the prompt field.
                    </p>
                    {jdFileName && (
                      <p className="mt-2 text-xs font-semibold text-[#39705b]">
                        Loaded: {jdFileName}
                      </p>
                    )}
                  </div>
              ...
              <div className="flex items-end">
                <button
                  ...
                  className="w-full rounded-lg bg-[#164d48] px-5 py-3 font-semibold text-white disabled:opacity-50"
                >
                  {state === 'loading'
                    ? 'Finding candidates…'
                    : 'Find candidates'}
            ...
            {state === 'error' && (
              <div className="mt-4 max-w-5xl rounded-lg bg-red-50 p-4 text-sm text-red-700">
                {error}
              </div>
            )}
            {screen === 'results' && (
            ...
            {selected && (
              <div className="fixed inset-y-0 right-0 w-[min(420px,100vw)] overflow-y-auto border-l border-[#dce6e2] bg-white p-7 shadow-xl">
                <button
                  onClick={() => setSelected(null)}
                  className="float-right text-sm text-slate-500"
                >
                  Close
                </button>
                <div className="mt-8 text-xs font-semibold text-[#c2764f]">
                  {selected.label}
                </div>
                <h2 className="mt-2 text-2xl font-semibold">{selected.name}</h2>
                <p className="text-slate-600">{selected.title}</p>
                <Detail
                  label="ATS score"
                  value={`${selected.atsScore} / 100`}
                />
                <Detail
                ...
                <Detail
                  ...
                  value={selected.matchedSkills.join(', ') || 'None'}
                />
                <Detail
                  label="Missing skills"
                  value={selected.missingSkills.join(', ') || 'None'}
                />
                <Detail
...
function Nav({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full rounded-lg px-4 py-3 text-left font-medium ${active ? 'bg-[#e4f1e9] text-[#176052]' : 'text-slate-600 hover:bg-slate-50'}`}
    >
      {children}
    </button>
  );
